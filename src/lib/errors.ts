...
export const ApiErrors = {
  ...
  // Conflict errors
  uniqueViolation: (message: string = "Resource already exists") =>
    new ApiError("unique_violation", message, 409),

  conflict: (message: string = "Resource conflict") =>
    new ApiError("conflict", message, 409),

  // Business logic errors
  jobInProgress: (message: string = "Job already in progress") =>
    new ApiError("job_in_progress", message, 409),

  cannotCancel: (message: string = "Cannot cancel job in current state") =>
    new ApiError("cannot_cancel", message, 422),

  limitExceeded: (message: string = "Resource limit exceeded") =>
    new ApiError("limit_exceeded", message, 413),

  // External service errors
  invalidKey: (message: string = "Invalid API key") =>
    new ApiError("invalid_key", message, 400),

  quotaExceeded: (message: string = "Quota exceeded") =>
    new ApiError("quota_exceeded", message, 402),

  ttsTimeout: (message: string = "TTS service timeout") =>
    new ApiError("tts_timeout", message, 504),
