...
async function testColumnLength() {
  ...
  try {
    ...
    try {
      const { error: error1 } = await supabase.from("tts_credentials").upsert({
    ...
    try {
      const { error: error2 } = await supabase.from("tts_credentials").upsert({
        user_id: "0a1f3212-c55f-4a62-bc0f-4121a7a72283",
      ...
      if (error2) {
        ...
        console.log(
          `JSON match: ${JSON.stringify(data2?.encrypted_key) === JSON.stringify(jsonObject) ? "YES" : "NO"}`
        );
      }
    } catch (e) {
