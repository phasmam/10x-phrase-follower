...
> _Notka_: “users” jako profil cienki do referencji i RLS; dane uwierzytelniania w `auth.users`.
...
> _Semantyka_: `is_active=true` oznacza segment należący do bieżącego buildu notatnika; zapewnione przez logikę + indeks/unikalność częściową (patrz §3).
...
- _USING_: `id = auth.uid()`
- _WITH CHECK_: `id = auth.uid()`

**notebooks**

- _USING_: `user_id = auth.uid()`
- _WITH CHECK_: `user_id = auth.uid()`

**phrases**

- _USING_: `EXISTS (SELECT 1 FROM notebooks n WHERE n.id = phrases.notebook_id AND n.user_id = auth.uid())`
- _WITH CHECK_: jak wyżej
