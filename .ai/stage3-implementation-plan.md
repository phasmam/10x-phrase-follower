...
- **Auth/RLS:** egzekwowane na wszystkich tabelach; w DEV tryb `DEV_JWT` z obejściem RLS _tylko lokalnie_ (middleware + service-role). W PROD wyłącznie JWT Supabase + RLS. (Źródło wewn.: `<auth>`; oraz polityki RLS w DB planie).
